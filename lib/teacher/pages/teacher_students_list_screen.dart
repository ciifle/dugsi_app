...
const Color _kSoftBlue = teacherWebBg;
...
class _TeacherStudentsListScreenState extends State<TeacherStudentsListScreen> {
  ...
  List<TeacherAssignedClassModel> get _assignedClasses {
    if (_dashboard?.assignedClasses != null &&
        _dashboard!.assignedClasses.isNotEmpty) {
    ...
    for (final a in _dashboard?.assignments ?? []) {
      if (a.classId > 0 && seen.add(a.classId)) {
        out.add(
          TeacherAssignedClassModel(id: a.classId, name: a.classDisplayName),
        );
      }
    }
    return out;
  ...
  Future<void> _loadDashboardThenStudents() async {
    ...
    debugPrint(
      'TeacherStudentsListScreen: assigned classes: ${_assignedClasses.length}',
    );
    await _loadStudents();
  }
  ...
  Future<void> _loadStudents() async {
    ...
    if (!mounted) return;
    debugPrint(
      'TeacherStudentsListScreen: students fetched: ${combined.length}',
    );
    setState(() {
      _loading = false;
  ...
  Widget build(BuildContext context) {
    final listSlivers = _buildListSlivers();
              ...
              child: Column(
                ...
                children: [
                  Icon(
                    ...
                    size: 56,
                    color: _kErrorColor.withOpacity(0.8),
                  ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          if (item.student.emisNumber != null &&
                              item.student.emisNumber!.isNotEmpty)
                            Text(
